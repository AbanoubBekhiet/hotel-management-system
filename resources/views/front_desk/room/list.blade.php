...
<body>
    <div>
        <form action="" method="get" class="max-w-7xl mx-auto flex flex-wrap gap-6">
            ...
            <div class="mb-5 w-[20%]">
                <label for="Price Per Neight" class="block mb-2 text-sm font-medium text-red-700 dark:text-red-500">Price Per Neight</label>
                <input type="number" name="price_per_neight" id="Price Per Neight"  value="{{Request::get('price_per_neight')}}" class="bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-sm rounded-lg focus:ring-red-500 dark:bg-gray-700 focus:border-red-500 block w-full p-2.5 dark:text-red-500 dark:placeholder-red-500 dark:border-red-500" placeholder="Price Per Neight">
            </div>
            <div class="mb-5 w-[20%]">
                <label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">VIB</label>
                <select id="countries" name="VIB" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                  <option value="">Choose a Room</option>
                  <option value="1" {{Request::get('VIB') == '1' ? 'selected' : ''}} >VIB</option>
                  <option value="0" {{Request::get('VIB') == '0' ? 'selected' : ''}} >Not VIB</option>
                </select>
            </div>
            <div class="mb-5 w-[20%]">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Price Per Neight</th>
                    <th scope="col" class="px-6 py-3">VIB</th>
            ...
            @foreach ($rooms as $room)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{$room->price_per_neight}}</td>
                    <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{$room->is_VIB == 1 ? 'VIB' : "Is not VIB"}}</td>
                    ...
                    <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        <a href="{{url('admin/assign_subject/edit/'.$room->id)}}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" >Edit</a>
                        <a href="{{url('admin/assign_subject/delete/'. $room->id)}}" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete</a>
                    </td>
                </tr>
            @endforeach
        </table>
